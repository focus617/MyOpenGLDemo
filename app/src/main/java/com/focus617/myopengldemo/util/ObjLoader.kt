...
//    var mtlMap: HashMap<String, MtlInfo>? = null      // 全部材质列表

    private var currentObjInfo = ObjInfo()
    ...
    fun load(context: Context, objFileName: String) {
        ...
        currentObjInfo.dump()
        currentObjInfo.dumpVertexList()
        currentObjInfo.dumpNormalList()
        currentObjInfo.dumpTextureList()
        currentObjInfo.dumpFaceList()
    ...
    private fun fillMtlLib(line: String) {
        ...
        if (!TextUtils.isEmpty(items[1])) {
            currentObjInfo.mMtlFileName = items[1]
    ...
    /**
     * build [ObjInfo.mVertexList] based on line from OBJ containing vertex data
     */
    private fun fillVertexList(line: String) {
        ...
        currentObjInfo.mVertexList.add(ObjVertex(x, y, z))
    }

    /**
     * build [ObjInfo.mNormalList] based on line from OBJ containing vertex data
     */
    private fun fillNormalList(line: String) {
        ...
        currentObjInfo.mNormalList.add(ObjNormal(x, y, z))
    }

    /**
     * build [ObjInfo.mTextureCoordList] based on line from OBJ containing vertex data
     *
    ...
    private fun fillTextureCoordList(line: String) {
        ...
        when (coordinates.size) {
            3 -> currentObjInfo.textureDimension = 2
            4 -> currentObjInfo.textureDimension = 3
            !in 3..4 -> return
        }
        ...
        currentObjInfo.mTextureCoordList.add(objTexture)
    }
    ...
    private fun fillFaceList(line: String) {
        ...
        val objFace =
            ObjFace(currentTextureName, defaultElement, defaultElement, defaultElement)

        if (!(vertexIndices[1].contains("/"))) {
            // vertexIndices[] format: "f vertexIndex1 vertexIndex2 vertexIndex3"
            currentObjInfo.hasNormalInFace = false
            currentObjInfo.hasTextureInFace = false

            for (i in 1 until clamp(vertexIndices.size + 1, 2, 4)) {
                objFace.put(
                    i - 1,
                    ObjFaceElement(Integer.valueOf(vertexIndices[i]) - 1, 0, 0)
                )
            }

        } else {
            // vertexIndices[] format: "f vertexIndex/textureIndex/normalIndex .."
            currentObjInfo.hasNormalInFace = true
            currentObjInfo.hasTextureInFace = true
        ...
        currentObjInfo.mFaceList.add(objFace)
    }
    ...
    class ObjInfo {
        ...
        //存放解析出来的纹理坐标
        val mTextureCoordList = ArrayList<ObjTexture>()
        ...
        fun dump() {
            ...
            Timber.d("Texture Array Size: ${mTextureCoordList.size}")
